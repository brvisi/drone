...
HMC5883L::HMC5883L()
{
	...
	writeTo(HMC5883L_ConfigurationRegisterA, 0x70);
...
void HMC5883L::read()
{
	...
	orientationVector[0] = (((int)_buff[0]) << 8) | _buff[1]; // X
	orientationVector[2] = (((int)_buff[2]) << 8) | _buff[3]; // Z
	orientationVector[1] = (((int)_buff[4]) << 8) | _buff[5]; // Y
